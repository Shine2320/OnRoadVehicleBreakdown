...
//                district=e10.getText().toString();
                ...
                else if(user.equalsIgnoreCase("")|| user.length()<4 || user.length()>10)
                {
                    e8.setError("Enter your username");
                    e8.setFocusable(true);
                }
                else if(password.equalsIgnoreCase("")|| password.length()<6 || password.length()>16)
                {
                    ...
                    String q ="/userregister?fname="+fname+"&lname="+lname+"&place="+place+"&phone="+phone+"&email="+email+"&username="+user+"&password="+password+"&latitude="+LocationService.lati+"&longitude="+LocationService.logi;
                    q = q.replace(" ","%20");
                    JR.execute(q);
                }
            }
